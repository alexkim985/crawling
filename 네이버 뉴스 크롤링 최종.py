...
def makePgNum(num): #예를들어 3--> 21 반환
    if num == 1:
        return num
    elif num == 0:
        return num+1
...
def makeUrl(search, start_pg, end_pg, start_date, end_date):
    if start_pg == end_pg:
        ...
        url = "https://search.naver.com/search.naver?where=news&sm=tab_pge&query=" + search + "&start=" + str(start_page)+'&sort=1'+f'&pd=3&ds={start_date}&de={end_date}'
        # query --> 검색어, start = 1, 11, 21 --> 1페이지, 2페이지, 3페이지, sort=1 --> 최신순. sort=0이면 관련도순
        print("생성url: ", url)
        url = [url]
        return url
    else:
        ...
        for i in range(start_pg, end_pg + 1):
            page = makePgNum(i)
            url = "https://search.naver.com/search.naver?where=news&sm=tab_pge&query=" + search + "&start=" + str(page)+'&sort=1'+f'&pd=3&ds={start_date}&de={end_date}'
            urls.append(url)
        print("생성url: ", urls)
        return urls    
...
start_date = input("\n검색할 시작 날짜를 입력하세요. ex)2023.01.03(연도.월.일):") # ex)1 =1페이지,2=2페이지...
print("\n크롤링할 종료 페이지: ",page2,"페이지")   
end_date = input("\n검색할 종료 날짜를 입력하세요. ex)2023.01.03(연도.월.일):") # ex)1 =1페이지,2=2페이지...
print("\n크롤링할 종료 페이지: ",page2,"페이지")   
...
for search in search_List :
    ...
    # naver url 생성
    urls = makeUrl(search,page,page2,start_date,end_date)
    ...
    for url in tqdm(naver_urls):
        ...
        news_info = news_html.select("div.media_end_head_top")
        news_info = news_info[0].img['title']
    ...
    #데이터 프레임 만들기
    news_df = pd.DataFrame({'date':news_dates,'title':news_titles,'link':naver_urls, 'info':news_infos,'content':news_contents, 'thumbnail':news_thumbnails})
